...
export function InfrastructureArchitectureNav() {
  ...
  const onOverview = pathname === "/knowledge/infrastructure-architecture";

  return (
    <nav
      className="rounded-2xl border border-slate-800/90 bg-slate-900/35 p-4"
      aria-label="Infrastructure and architecture navigation"
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
          ...
          <span className="text-slate-400">Infrastructure & architecture</span>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {infrastructureArchitecturePages.map((p) => {
            const href = `/knowledge/infrastructure-architecture/${p.slug}`;
