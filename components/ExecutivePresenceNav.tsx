...
export function ExecutivePresenceNav() {
  ...
  const onOverview = pathname === "/knowledge/executive-presence";

  return (
    <nav
      className="rounded-2xl border border-slate-800/90 bg-slate-900/35 p-4"
      aria-label="Executive presence and boardroom navigation"
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
          ...
          <span className="text-slate-400">Executive presence</span>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {executivePresencePages.map((p) => {
            const href = `/knowledge/executive-presence/${p.slug}`;
