...
export function FullMockInterviewPanelNav() {
  ...
  const onOverview = pathname === "/knowledge/full-mock-interview-panel";

  return (
    <nav
      className="rounded-2xl border border-slate-800/90 bg-slate-900/35 p-4"
      aria-label="Full mock interview panel navigation"
    >
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
          ...
          <span className="text-slate-400">Full mock interview panel</span>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {fullMockInterviewPanelPages.map((p) => {
            const href = `/knowledge/full-mock-interview-panel/${p.slug}`;
