...
export function BehavioralPressureNav() {
  ...
  const onOverview = pathname === "/knowledge/behavioral-pressure";

  return (
    <nav
      className="rounded-2xl border border-slate-800/90 bg-slate-900/35 p-4"
      aria-label="Behavioural and pressure questions navigation"
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
          ...
          <span className="text-slate-400">Behavioural & pressure</span>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {behavioralPressurePages.map((p) => {
            const href = `/knowledge/behavioral-pressure/${p.slug}`;
