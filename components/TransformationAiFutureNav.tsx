...
export function TransformationAiFutureNav() {
  ...
  const onOverview = pathname === "/knowledge/transformation-ai-future";

  return (
    <nav
      className="rounded-2xl border border-slate-800/90 bg-slate-900/35 p-4"
      aria-label="Transformation, AI and future strategy navigation"
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
          ...
          <span className="text-slate-400">Transformation &amp; AI</span>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {transformationAiFuturePages.map((p) => {
            const href = `/knowledge/transformation-ai-future/${p.slug}`;
