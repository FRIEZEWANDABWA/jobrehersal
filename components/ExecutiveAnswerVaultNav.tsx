...
export function ExecutiveAnswerVaultNav() {
  ...
  const onOverview = pathname === "/knowledge/executive-answer-vault";

  return (
    <nav
      className="rounded-2xl border border-slate-800/90 bg-slate-900/35 p-4"
      aria-label="Executive answer vault navigation"
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
          ...
          <span className="text-slate-400">Executive answer vault</span>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {executiveAnswerVaultPages.map((p) => {
            const href = `/knowledge/executive-answer-vault/${p.slug}`;
